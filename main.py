...
def restart_after_5_hours():
    """Restart the whole Python process every 5 hours."""
    RESTART_INTERVAL = 5 * 60 * 60  # 5 hours
    time.sleep(RESTART_INTERVAL)
    print(f"{YELLOW}[RESTART] 5 hours reached - restarting process...{RESET}", flush=True)
...
def run_api():
    ...
    # Automatic full process restart every 5 hours.
    threading.Thread(target=restart_after_5_hours, daemon=True).start()
