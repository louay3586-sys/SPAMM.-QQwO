...
def thread_monitor():
    """Diagnostic monitor: report the current number of active Python threads."""
    while True:
        try:
            print(
                f"[THREAD MONITOR] active_threads={threading.active_count()}",
                flush=True
            )
        except Exception as e:
            print(f"[THREAD MONITOR] error: {e}", flush=True)
        time.sleep(10)
...
def run_api():
    ...
    # Diagnostic only: print active thread count every 10 seconds.
